...
namespace CACES.BLL.Servicios.Auth
{
    public class AuthServicio : IAuthServicio
    {
        ...
        public async Task<(bool Exito, string Token, string Mensaje)>
            GenerarTokenRecuperacionAsync(OlvidoContrasenaDTO dto)
        {
            var usuario = await _usuarioRepositorio
                .GetUsuarioByEmailAsync(dto.CorreoElectronico);
            ...
            if (usuario == null || usuario.Estado == false)
            {
                return (
                    true,
                    string.Empty,
                    "Si el correo coincide con una cuenta activa, se enviarán las instrucciones."
                );
            }
        ...
        public async Task<(bool Exito, string Mensaje)>
            RestablecerContraseñaAsync(RestablecerContrasenaDTO dto)
        {
            var usuario = await _usuarioRepositorio
                .GetUsuarioByEmailAsync(dto.CorreoElectronico);
            ...
            if (string.IsNullOrEmpty(usuario.SecurityStamp) ||
                usuario.SecurityStamp != dto.Token)
            {
                return (
                    false,
            ...
            bool actualizado =
                await _usuarioRepositorio.UpdateUsuarioAsync(usuario);
